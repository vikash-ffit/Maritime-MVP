...
export default function WorkRestGrid() {
  const [days, setDays] = useState<DailyWorkRest[]>(
    Array.from({ length: 30 }, (_, i) => {
      ...
      if (i === 0) { // June 1
        blocks = createBlocks([[16, 26], [32, 40], [40, 48]]); // ~13.5 hours
        remarks = "Arrival at Turkey, Berthing.";
      } else if (i === 1) { // June 2
        blocks = createBlocks([[16, 27], [30, 48]]); // ~14.5 hours
        remarks = "Comoensation will be given next day.";
        watchKeeper = true;
      } else if (i === 2) { // June 3
        blocks = createBlocks([[16, 27], [40, 48]]); // ~10.5 hours
      } else if (i === 3) { // June 4
        blocks = createBlocks([[4, 10], [12, 48]]); // ~19.0 hours
        watchKeeper = true;
      }
  ...
  const handleBlockToggle = (dayIndex: number, blockIndex: number, forceValue?: boolean) => {
    setDays((prev) => {
      const newDays = [...prev];
      const newBlocks = [...newDays[dayIndex].blocks];
      const newValue = forceValue !== undefined ? forceValue : !newBlocks[blockIndex];
  ...
  return (
    <div className="bg-white dark:bg-slate-950 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
      <div className="overflow-x-auto custom-scrollbar" style={{ cursor: isDragging ? 'ew-resize' : 'default' }}>
        ...
        <div className="flex items-stretch min-w-max bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800">
              ...
              {Array.from({ length: 24 }).map((_, h) => (
                ...
                >
                  ...
                  <div className="flex w-full divide-x divide-slate-200 dark:divide-slate-700">
                    <div className="flex-1 text-center text-[9px] font-bold text-slate-500 py-[2px]">00</div>
                    <div className="flex-1 text-center text-[9px] font-bold text-slate-500 py-[2px]">30</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="flex items-center gap-4 p-3 pr-6 ml-auto border-l border-slate-200 dark:border-slate-800/50">
            <div className="w-12 text-center text-[10px] font-bold text-slate-500 uppercase">Sea / Port</div>
            <div className="w-12 text-center text-[10px] font-bold text-slate-500 uppercase">Watch</div>
            <div className="w-12 text-center text-[10px] font-bold text-slate-500 uppercase">US Line</div>
            <div className="w-16 text-center text-[10px] font-bold text-slate-500 uppercase">Work 24H</div>
            <div className="w-16 text-center text-[10px] font-bold text-slate-500 uppercase">Rest 24H</div>
            <div className="w-20 text-center text-[10px] font-bold text-slate-500 uppercase">Rest 7D</div>
            <div className="w-64 text-[10px] font-bold text-slate-500 uppercase pl-2">Remarks</div>
          </div>
        </div>
